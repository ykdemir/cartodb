...
cdb.admin.mod.LegendEditor = cdb.core.View.extend({
  ...
  _setupBindings: function() {
    ...
    this.options.dataLayer.bind("change:tile_style", this._onStyleChange, this);
  ...
  _onWizardChange: function() {
    ...
    if (type == 'polygon') {
      var tab = this.panes.getActivePane();
      if (tab.model.get("type") == "custom") type = 'custom';
    }

    if (!_.contains(this._getPanes(), type)) {
      this._enableNoneLegend();
    } else {
      this._enablePane(type);
    }
