...
cdb.admin.mod.LegendEditorItem = cdb.core.View.extend({
  ...
  initialize: function() {
    ...
    _.bindAll(this, "_onToggleShowTitle", "_onToggleSync");
  ...
  _onToggleSync:function() {
    ...
    if (this.model.get("sync")) this.$el.find(".checkbox.sync").addClass("enabled").removeClass("disabled");
    else this.$el.find(".checkbox.sync").addClass("disabled").removeClass("enabled");
  ...
  _addEditInPlace: function() {
    ...
    this.editInPlace.bind("change", function(val) {
      ...
      console.log(cdb._debugCallbacks(this.model));
    })
