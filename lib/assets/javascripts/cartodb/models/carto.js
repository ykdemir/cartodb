...
function color_generator(table, props, callback) {
  var type = table.geomColumnTypes() && table.geomColumnTypes()[0] || "polygon";
  ...
  // Set values color
  //
  // Unfortunately, we have to check if property
  // exists in the table to prevent tiler fails.
  // It could happen when user deletes the column
  // applied
  if (props.colors && table.containsColumn(props.property)) {
    // type of the column, number -> no quotes, string -> quotes, boolean -> no quotes
    for (var i in props.colors) {
      if (props.colors[i][3]) {
        // Default color
        css += "\n#" + table_name + " {\n   " + fill + ": " + props.colors[i][1] + ";\n}";
      } else {
        // Set correct value type
        var value = '';
        if (props.colors[i][2] != "string" || props.colors[i][0] === null) {
          value = props.colors[i][0];
        } else {
          value = "\"" + props.colors[i][0] + "\"";
        }
...
cdb.admin.CartoStyles = Backbone.Model.extend({
    ...
    _generateCarto: function(){
      ...
      gen(this.table, this.get('properties'), function(style, metadata) {
  ...
  _extract: function(method, extractVariables) {
    ...
    if (this.ruleset) {
      ...
      for (var d in definitions) {
        ...
        if (def.filters) { // extract from rules
