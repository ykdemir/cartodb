...
describe('widgets', function() {
  ...
  describe('cdb.forms.ColorPicker', function() {
    var view, defaultColors;

    beforeEach(function() {

      defaultColors = ["#cccccc", "#f1f1f1"];
    ...
    it("should generate the colors", function() {
      view.render();

      var colors = _.map(view.$el.find("ul.colors li a"), function(el) {
        var rgbColor = $(el).css("background");
        return rgb2hex(rgbColor);
      });

      expect(_.isEqual(colors, defaultColors)).toBeTruthy();

    });

    it("should update the colors", function() {
      view.render();

      var newColors = ["#ff000", "#0000ff"];
      view.setColors(newColors);
      ...
      expect(_.isEqual(colors, newColors)).toBeTruthy();
